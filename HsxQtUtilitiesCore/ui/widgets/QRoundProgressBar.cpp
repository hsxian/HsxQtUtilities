...
void QRoundProgressBar::paintEvent(QPaintEvent * /*event*/)
{
    ...
    //画背景矩形填充白色
    p.fillRect(baseRect, QBrush(getBackground()));
...
void QRoundProgressBar::drawBase(QPainter &p, const QRectF &baseRect, const QRectF &innerRect)
{
    switch(m_barStyle)
    {
        case StyleDonut:
        {
            ...
            QPen pen;
            pen.setColor(getColor4Base());
            pen.setWidth(m_dataPenWidth);
            p.setPen(pen);
            p.setBrush(QBrush(getBackground4InnerCircle()));
        ...
        case StylePie:
        {
            p.setPen(QPen(QColor(getBackground4InnerCircle()), m_outlinePenWidth));
            p.setBrush(QBrush(getColor4Base()));
    ...
    if(m_barStyle == StyleLine)
    {
        ...
        else
        {
            ...
            if(m_barStyle == StylePie)
            {
                ...
                p.setPen(QPen(getColor4Value(), m_dataPenWidth));
            }
            p.setBrush(QBrush(getColor4Value()));
            p.drawPath(dataPath);
        }
...
void QRoundProgressBar::drawText(QPainter &p, const QRectF &rect, double value)
{
    ...
    p.setFont(f);
    p.setPen(getColor4Text());

    p.drawText(rect, Qt::AlignCenter, getPercentageSymbolVisible() ? textToDraw : getText());
}
