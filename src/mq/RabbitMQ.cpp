...
RabbitChannelPtr RabbitMQHelper::channelInstance(amqp_channel_t channel)
{
...
amqp_channel_t RabbitMQHelper::getChannelId()
{
...
int RabbitMQHelper::freeChannelId(amqp_channel_t channel)
{
...
std::atomic_bool RabbitMQHelper::getIsBasicConsummingMessage() const
{
    return m_isBasicConsummingMessage;
}
...
RabbitChannel::RabbitChannel(amqp_channel_t channel, RabbitMQHelper& mqHelper)
...
bool RabbitChannel::isChannelOpen()
{
    return is_connected_;
}

void RabbitChannel::closeChannel()
{
    if(!is_connected_)
    {
        return;
...
void RabbitChannel::closeConnection()
{
            ...
            if(AMQP_FRAME_METHOD == frame.frame_type)
            {
                switch(frame.payload.method.id)
                {
                    ...
                    case AMQP_CHANNEL_CLOSE_METHOD:
                        ...
                        LOG_API "Close other channel:" << frame.channel;
...
RabbitMQHelper::RabbitMQHelper(const std::string& connect_uri, int frame_max):
...
RabbitMQHelper::RabbitMQHelper(const std::vector<std::string>& connect_uris, int frame_max):
...
bool RabbitMQHelper::doConnect(const QString& host, int port, const QString& user, const QString& password, const QString& vhost)
{
    if(frame_max_ <= 0)
    {
    ...
    auto hostname = host.toStdString().c_str();
    auto username = user.toStdString().c_str();
    auto pw = password.toStdString().c_str();
    auto vh = vhost.toStdString().c_str();

    amqp_socket_t* socket = amqp_tcp_socket_new(connection_);
    int sock = amqp_socket_open(socket, hostname, port);
    if(sock < 0)
    {
    ...
    amqp_rpc_reply_t res = amqp_login(connection_, vh, 0, frame_max_, 0,
                                      AMQP_SASL_METHOD_PLAIN, username, pw);
    if(AMQP_RESPONSE_NORMAL != res.reply_type)
    {
    ...
    LOG_API("rabbitmq client connect to %s:%d/%s ok!", hostname, port, vh);
    is_connected_ = true;
...
bool RabbitMQHelper::isConnectionOpen()
{
    return is_connected_;
}
...
bool mq_parse_connect_uris(const std::string& uris, const std::string& passwd, std::vector<std::string>& conn_vec)
{
    ...
    auto  rawstr = QString::fromStdString(uris);
    auto array = rawstr.split(";");
...
bool mq_setup_channel_consume_default(RabbitChannelPtr pChannel, void* pArg)
{
    ...
    if(pChannel->basicQos(1000, true) < 0)
    {
...
RabbitMessage::RabbitMessage() : dirt(false)
{
...
amqp_bytes_t RabbitMessage::content()
{
    return envelope.message.body;
}

bool RabbitMessage::has_content()
{
    return !!(envelope.message.body.len);
}

void RabbitMessage::safe_clear()
{
