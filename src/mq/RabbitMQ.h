...
#define LOG_API qDebug()<<__FUNCTION__<<__LINE__<<
...
struct HSXQTUTILITIES_EXPORT RabbitMessage
{
public:
    RabbitMessage();
...
class HSXQTUTILITIES_EXPORT RabbitMQHelper
{
...
public:
    ...
    bool doConnect(const QString& host, int port, const QString& user, const QString& password, const QString& vhost = "/");
    std::atomic_bool getIsBasicConsummingMessage() const;
...
private:
    ...
    std::atomic_bool m_isBasicConsummingMessage = std::atomic_bool{false};
...
class HSXQTUTILITIES_EXPORT RabbitChannel
{
public:
...
bool HSXQTUTILITIES_EXPORT mq_setup_channel_consume_default(RabbitChannelPtr pChannel, void* pArg);
